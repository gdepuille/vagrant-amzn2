...
source "virtualbox-ovf" "autogenerated_1" {
  ...
  vboxmanage           = [
    ...
    ["storageattach", "{{.Name}}", "--storagectl", "IDE Controller", "--port", "0", "--device", "0", "--type", "dvddrive", "--medium", "target/seed.iso"]
  ]
...
build {
  ...
  provisioner "shell" {
    execute_command = "sudo chmod +x {{ .Path }}; {{ .Vars }} sudo -E {{ .Path }}"
    scripts = [
